var campaignID = "68f693769c3146d48c8e82d0";
...
var initialSrc = "https://atlls.remeltpounds.com/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("atlls.remeltpounds.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://atlls.remeltpounds.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://atlls.remeltpounds.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("atlls.remeltpounds.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
